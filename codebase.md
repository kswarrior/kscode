...
+- rebuild.sh                 clean-rebuild script -> ./release/ (backend + web)
+- workspace/                 default on-disk workspace (edited by the server)
+- release/                   (generated by rebuild.sh) kscode-server + web/ + workspace/
|
...
- Output layout: `release/kscode-server`, `release/web/`, `release/workspace/` - run via
  `KS_STATIC=release/web KS_WORKSPACE=release/workspace KS_API_DIR=release/data ./release/kscode-server`.
